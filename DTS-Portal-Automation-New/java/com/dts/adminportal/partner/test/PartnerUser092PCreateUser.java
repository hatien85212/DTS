...
public class PartnerUser092PCreateUser extends BasePage{
	...
	public void TC092PaCUB_01(){
		...
		Assert.assertTrue(ListUtil.containsAll(privileges, AddUser.PRIVILEGES));
	}
	...
	public void TC092PaCUB_03(){
		 ...
		 */
		Assert.assertTrue(userControl.checkMessageDisplay("! Account with this email address already exists"));
	...
	public void TC092PbCUF_02(){
		...
		userControl.addErrorLog("PDPP-1454 - 092 Add new user: New user is able to created without assigning privileges.");
		/*		
		1. Navigate to DTS portal
		2. Log into DTS portal as a partner user successfully
		3. Click "Users" tab
		...
		9. Enable a site privilege
		10. Disable site notification of above privilege
		11.  Click "Save" link
		...
		*/
		...
		Assert.assertTrue(userControl.isNotificationCheckboxEnable(AddUser.PRIVILEGES_TABLE, Privileges.Add_and_manage_products));
		Assert.assertTrue(userControl.isNotificationCheckboxEnable(AddUser.PRIVILEGES_TABLE, Privileges.Edit_brand_info));
		Assert.assertTrue(userControl.isNotificationCheckboxEnable(AddUser.PRIVILEGES_TABLE, Privileges.Publish_and_suspend_products));
		Assert.assertTrue(userControl.isNotificationCheckboxEnable(AddUser.PRIVILEGES_TABLE, Privileges.Request_product_tunings));
		...
		Assert.assertFalse(userControl.isNotificationCheckboxEnable(AddUser.PRIVILEGES_TABLE, Privileges.Add_and_manage_products));
		Assert.assertFalse(userControl.isNotificationCheckboxEnable(AddUser.PRIVILEGES_TABLE, Privileges.Edit_brand_info));
		Assert.assertFalse(userControl.isNotificationCheckboxEnable(AddUser.PRIVILEGES_TABLE, Privileges.Publish_and_suspend_products));
		Assert.assertFalse(userControl.isNotificationCheckboxEnable(AddUser.PRIVILEGES_TABLE, Privileges.Request_product_tunings));
		
		//8. Click "Save" link
		userControl.click(AddUser.SAVE);
		//VP: New user could not be created successfully
		//TODO: Below section should be optimized again after issue is resolved
		Assert.assertFalse(userControl.isElementPresent(PageHome.userCreateSucess));
	...
	public void TC093PCUS_07() {
		...
		/*
			...
			VP: The Partner activation page is launched with ï¿½Emailï¿½, ï¿½Passwordï¿½, and ï¿½Confirm Passwordï¿½ fields
		...
		*/
		...
		// Delete user if exist
		 if (userControl.checkUserExistByEmail(PARTNER_DTS_EMAIL)){
		...
		/*
		 * Verify that The Partner activation page is launched with ï¿½Emailï¿½,
		 * ï¿½Passwordï¿½, and ï¿½Confirm Passwordï¿½ fields
		 */
		...
		userControl.activeUser(PARTNER_DTS_EMAIL, NEW_ACTIVE_USER_PASSWORD);
		/*
		 * Verify that An error message ï¿½Cannot active account, the link may be not valid or expriedï¿½ displays
		 */
