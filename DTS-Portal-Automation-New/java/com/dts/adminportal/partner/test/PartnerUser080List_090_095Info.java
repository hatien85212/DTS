...
public class PartnerUser080List_090_095Info extends BasePage{
	...
	public void TC080PUL_07(){
		...
		productControl.selectReportsFilterByText(UsersList.FILTER_USER, UsersList.company_filter[1]);
		 ...
		 */
		Assert.assertTrue(userControl.checkAllUserStatus(UsersList.company_filter[1]));
		//5. Set the Account filter dropdown to "Invited"
		productControl.selectReportsFilterByText(UsersList.FILTER_USER, UsersList.company_filter[2]);
		/*
		 * The users table shows up only activated users and all their information are correctly
		 */
		Assert.assertTrue(userControl.checkAllUserStatus(UsersList.company_filter[2]));
		
		//6. Set the Account filter dropdown to "All Accounts"
		productControl.selectReportsFilterByText(UsersList.FILTER_USER, UsersList.company_filter[0]);
		//home.selectFilterByName(UsersList.FILTER_USER, user_status);
		/*
		 * The users table shows up only activated users and all their information are correctly
		 */
		Assert.assertTrue(userControl.checkAllUserStatus(UsersList.company_filter[0]));
...
//		Assert.assertTrue(confirm[0] == null);
	...
	public void TC095PUI_02(){
		...
		Assert.assertTrue(userControl.isNotificationCheckboxSelected(PartnerUserMgmt.SITE_PRIVILEGES_TABLE, Privileges.privileges[0]));
	...
	public void TC095PUI_06(){
		...
		// 6. Change user own email address to another
		String newEmail = AddUser.NEW_EMAIL_ELEMENT[0] + RandomStringUtils.randomNumeric(5) + AddUser.NEW_EMAIL_ELEMENT[1];
		...
		Assert.assertEquals(userControl.getTextByXpath(PageLogin.ERROR_MESSAGE), PageLogin.errMessage[3]);
