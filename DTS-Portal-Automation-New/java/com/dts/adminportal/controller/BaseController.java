		...
		try {
			...
			for (int i = 1; i <= 10; i++) {
				...
				if (FileUtil.FileExist(file_path)) {
					...
					addLog("File: " + filename + " is not downloaded successfully");
					return false;
				}
			}			
			}	
...
//			autoTool.mouseWheel("up", 50);
//			// scroll to the element
//			int i = 0;
//			while(dragDropArea == null && i < 20){
//				autoTool.mouseWheel("down", 5);
//				Thread.sleep(300);
//				if (fileExt.equals("hpxtt") || fileExt.equals("zip")) {
//					target = new ImageTarget(new File(path + File.separator +  "TuningDragDropArea.png"));
//				} else if (fileExt.equals("jpg")) {
//					target = new ImageTarget(new File(path + File.separator +  "ImageDragDropArea" + fileName));
//				} else {
//					target = new ImageTarget(new File(path + File.separator +  "MarketingDragDropArea.png"));
//				}
...
//	}
	  ...
	  private boolean isWebElementVisibleOnWindows(WebElement w) {
	...
	public void dragDropFile(String fileName) {
		try {
//			WebElement dragdroptuningarea = Document.g
//					driver.findElement(By.xpath(AddEditProductModel.TUNING_DRAG_DROP_AREA));
			
//			driver.switchTo().activeElement().equals(driver.findElement(By.xpath(AddEditProductModel.TUNING_DRAG_DROP_AREA)));
//String n = driver.switchTo().activeElement().getLocation().toString();
//			System.out.println(n);
			
			//js.executeScript("function showAlert() { alert('success'); }; showAlert()");
			//js.executeScript("document.getElementById('dropzone-tuning').scrollIntoView(true);");
			ScreenRegion sreen = new DesktopScreenRegion();
			Mouse mouse = new DesktopMouse();
			ScreenRegion dragDropArea = null;
			Target target = null;
			...
			while(dragDropArea == null && i < 20){
				...
				if (fileExt.equals("hpxtt") || fileExt.equals("zip")) {
					...
					//System.out.println(isWebElementVisibleOnWindows(driver.findElement(By.xpath(AddEditProductModel.TUNING_DRAG_DROP_AREA))));
					//js.executeScript ("document.getElementById('dropzone-tuning').focus();");
					//js.executeScript("document.getElementByXpath(\""+ AddEditProductModel.TUNING_DRAG_DROP_AREA +"\").focus();");
