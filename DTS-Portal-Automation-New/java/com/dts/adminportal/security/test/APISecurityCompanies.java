...
public class APISecurityCompanies extends BasePage {
	...
	public void TCAPIC_01(){
		...
		userControl.disablePrivilege(AddUser.PRIVILEGES_TABLE, Privileges.Edit_company_info);
...
//		dataBrand = APIUtil.replaceStringValue(dataBrand, "brandName", "Brand-Security" + RandomStringUtils.randomNumeric(6));
		...
		// Disable "Edit Company Info" privileges
		userControl.enableAllPrivileges();
		userControl.disablePrivilege(AddUser.PRIVILEGES_TABLE, Privileges.Edit_brand_info);
		userControl.click(AddUser.SAVE);
		// Logout user
		userControl.logout();
		// Login DTS portal as DTS user above
	...
	public void TCAPIC_07(){
		...
		userControl.enableAllPrivileges();
		userControl.disablePrivilege(AddUser.DTS_PRIVILEGES_TABLE, Privileges.Add_manage_company_account);
		userControl.click(AddUser.SAVE);
		// Logout user
		userControl.logout();
		// Login DTS portal as DTS user above
...
/*
	...
	public void TCAPIC_13(){
		...
		JSONObject requestResult = APIUtil.sendGet(APIData.CHANGE_PRIMARY_CONTACT_URL(PARTNER_DTS_USER_SER_COMP_ID, PARTNER_DTS_USER_SER_ID), csrfToken, cookie);
		/*
		 * {"apiErrorCode":"PEMISSION_DENY","message":"You don't have permission."}
		 */
		Assert.assertEquals(requestResult.get("message").toString(), APIData.ERROR_MESSAGE);
	...
	public void TCAPIC_18(){
		...
		if (!requestResult.containsKey("message")) {
			userControl.addErrorLog("PDPP-1328: 092Db New user account form: DTS user is able to add new brand although the \"Edit Company Info\" privilege is disabled and \"Add and manage company accounts\" privilege is enabled");
		}
		/*
		 * {"apiErrorCode":"PEMISSION_DENY","message":"You don't have permission."}
		 */
