...
//		audioControl.doDelete(AudioRoutesEdit.DELETE_TUNING_ICON);
	...
	public void TC621DRE_05(){
		...
		// 7. Click on above tuning link after uploaded
		// 8. Try to download the tuning file
		boolean isDownloaded = audioControl.downloadFile(AddEditProductModel.FileUpload.Super_Stereo_Wide.getName());
		/*
	...
	public void TC621DRE_06() {
		...
		*/
		...
		WebElement Namefirst = driver.findElement(By.xpath("//*[contains(@id,'tuning-fileupload-')]/table/tbody/tr[2]/td"));
		String NameBefore = audioControl.getTextByXpath(Namefirst);
		...
		// VP: The tuning file still remains.
		WebElement Namelater = driver.findElement(By.xpath("//*[contains(@id,'tuning-fileupload-')]/table/tbody/tr[2]/td"));
		String NameAfter = audioControl.getTextByXpath(Namelater);
	...
	public void TC621DRE_10() {
		...
		/*
			...
			5. Click "Edit" link	
			VP: The 621D Room Model page is displayed
			6. Upload an invalid tuning file using invalid extension format	
			VP: An error message 'Invalid File' is displayed and the invalid tuning file could not be added successfully.
			7. Upload again using the audio route for Other Audio Route to upload	
			VP: An error message 'Invalid File' is displayed and the invalid tuning file could not be added successfully.
		...
		Assert.assertEquals(audioControl.existsElement(RoomModelEdit.getElementsInfo()), true);
		//6. Upload an invalid tuning file using invalid extension format
		audioControl.uploadFileTuning(RoomModelEdit.ADD_TUNING,AddEditProductModel.FileUpload.Tuning_HPXTT_LineOut.getName());
		/*
		 * VP: An error message 'Invalid File' is displayed and the invalid tuning file could not be added successfully.
		 */
		Assert.assertTrue(audioControl.getTextByXpath(AudioRoutesEdit.AUDIOROUTE_UPLOAD_MESSAGE).contains(AudioRoutesEdit.Upload_File_Message.Invalid_file.getName()));
		//7. Upload again using the audio route for Other Audio Route to upload	
		audioControl.uploadFileTuning(RoomModelEdit.ADD_TUNING,AddEditProductModel.FileUpload.Attached0InternalSpeakersDefault_Fallback.getName());
		/*
		 * VP: An error message 'Invalid File' is displayed and the invalid tuning file could not be added successfully.
		 */
		Assert.assertTrue(audioControl.getTextByXpath(AudioRoutesEdit.AUDIOROUTE_UPLOAD_MESSAGE).contains(AudioRoutesEdit.Upload_File_Message.Invalid_file.getName()));
	...
	public void TC621DRE_12() {
		...
		*/
		...
		audioControl.uploadFileTuning(RoomModelEdit.ADD_TUNING,AddEditProductModel.FileUpload.McRoom0_RC1_5p1_7p1.getName());
		//7. Click on Trashcan icon of tuning secion
		audioControl.click(RoomModelEdit.DELETE_TUNING);
		 ...
		 * VP: The tuning file still remains
		 */
		Assert.assertTrue(audioControl.isElementPresent(RoomModelEdit.TUNING_INFO_CONTAINER));
		//9. Click "Delete" button on delete confirmation dialog
		audioControl.click(RoomModelEdit.DELETE_TUNING);
		audioControl.waitForLoadElement(PageHome.BTN_CONFIRMATION_DANGER);
		audioControl.selectConfirmationDialogOption("Delete");
	...
	public void TC621DRE_14() {
		...
		*/
		...
		audioControl.click(AudioRoutes.MCROOM1);
		audioControl.editVersion();
		audioControl.uploadFileTuning(RoomModelEdit.ADD_TUNING,AddEditProductModel.FileUpload.MCRoom1.getName());
		audioControl.click(RoomModelEdit.SAVE);
		//3. Navigate to "Audio" page
		audioControl.click(PageHome.linkAudioroutes);
		//4. Select a multi-channel room model item.
	...
	public void TC621DRE_15() {
		...
		7. Upload again using the audio route for Other Audio Route to upload	
		VP: Verify that the "Invalid File" message and Retry link are displayed
		8. Click "Retry" link in "Tuning" section	
		9. Upload a valid tuning file.	
		VP: The valid tuning file is uploaded successfully.
		10. Click "Save" link	
		11. Select the model item above	
		...
		*/
		 ...
		 * VP: Verify that the "Invalid File" message and Retry link are displayed
		 */
		Assert.assertTrue(audioControl.getTextByXpath(RoomModelEdit.INVALID_ERR).contains(RoomModelEdit.error_Mess.Invalid_Tuningfile.getName()));
		Assert.assertTrue(audioControl.isElementPresent(RoomModelEdit.RETRY_ICON));
		//7. Upload again using the audio route for Other Audio Route to upload	
		audioControl.uploadFileTuning(RoomModelEdit.ADD_TUNING,AddEditProductModel.FileUpload.Attached0InternalSpeakersDefault_Fallback.getName());
		/*
		 * VP: Verify that the "Invalid File" message and Retry link are displayed
		...
		//8. Click "Retry" link in "Tuning" section	
		//9. Upload a valid tuning file.
		audioControl.uploadFileTuning(RoomModelEdit.RETRY_ICON,AddEditProductModel.FileUpload.Super_Stereo_Wide.getName());
		...
		//11. Select the model item above
		/*
		 * VP: The valid tuning file is displayed in the 616D Room Model Detail page.
		 */
	...
	public void TC621DRE_16() {
		...
		*/
		Assert.assertEquals(audioControl.getTextByXpath(RoomModelEdit.ERROR_MESSAGE),RoomModelEdit.error_Mess.Name_Required.getName());
		audioControl.click(RoomModelEdit.CANCEL);
		// 10. Repeat from step 4 to step 7 and change name value with out of maximum value characters
		//Click "Edit Version" link
		audioControl.click(RoomModelInfo.EDIT);
		...
		* VP: A error message is dispalyed like "Name exceeds 255 characters limit" and the Stereo Room Model cannot saved.
		*/
		Assert.assertEquals(audioControl.getTextByXpath(RoomModelEdit.ERROR_MESSAGE),RoomModelEdit.error_Mess.Name_255characterslimit.getName());
		// 11. Repeat from step 4 to step 10 with "Other Audio Routes" and "Multi-Channel Room Model" sections
		...
		* VP: A error message is dispalyed like "Name is required" and the Stereo Room Model cannot saved.
		*/
		Assert.assertEquals(audioControl.getTextByXpath(RoomModelEdit.ERROR_MESSAGE),AudioRoutesEdit.err_message.Name_is_required.getName());
		audioControl.click(RoomModelEdit.CANCEL);
		...
		*/
		Assert.assertEquals(audioControl.getTextByXpath(RoomModelEdit.ERROR_MESSAGE),AudioRoutesEdit.err_message.Name_with_character_limit.getName());	
