...
public class PartnerUser091PEdit_096ChangePassword extends BasePage{
	...
	public void TC091PEU_01(){
		...
		//VP: The 091 Edit User page is displayed with correct given name, family name, title, email, country node, phone, status, site privileges, brand privileges, notification of the selected user from users table.
		//VP: Verify that the 091P Edit Users is displayed and the "Actions" module contains "Save" and "Cancel" link
...
//		String retype_pass =BasePage.CHANGING_PASSWORD_MESSAGE[5];
		 ...
		 */
		Assert.assertEquals( userControl.getTextByXpath(PartnerChangePassword.requires.old_password.getName()), PartnerChangePassword.CHANGING_PASSWORD_MESSAGE.Old_password_is_required.getName());
		Assert.assertEquals( userControl.getTextByXpath(PartnerChangePassword.requires.new_password.getName()), PartnerChangePassword.CHANGING_PASSWORD_MESSAGE.Password_is_required.getName());
		Assert.assertEquals( userControl.getTextByXpath(PartnerChangePassword.requires.retype_password.getName()), PartnerChangePassword.CHANGING_PASSWORD_MESSAGE.Retype_New_Password_not_match.getName());		
		 ...
		 */
		Assert.assertEquals( userControl.getTextByXpath(PartnerChangePassword.requires.old_password.getName()), PartnerChangePassword.CHANGING_PASSWORD_MESSAGE.Old_Password_Not_Match.getName());
