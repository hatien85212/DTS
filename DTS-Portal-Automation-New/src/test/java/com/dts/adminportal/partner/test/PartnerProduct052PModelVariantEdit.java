...
//	}
	...
	public void TC052PMVE_01(){
		...
		/*
			...
			VP: 052 Model Variant Edit page is showed up and the Headphone: X Tuning Rating is hidden
...
//		VP: 052 Model Variant Edit page is showed up with: Model Name text field, Languages dropdown, EAN/UPC text field, Descriptor, Input Specifications, Release date, Tuning ( Use Parent's Tuning checkbox and Upload Custom Tuning link), Headphone: X Tuning Rating, Primary Catalog Image section and Marketing Material section.
		...
		Assert.assertFalse(productControl.isElementPresent(PageHome.HEADPHONE_TUNING_RATING));
...
//		VP: New tuning zip file is uploaded successfully and it's called "Partner Tuning"
		...
		Assert.assertEquals(productControl.getTextByXpath(AddEditProductModel.VARIANT_UPLOADED_TUNING_NAME), AddEditProductModel.tuningname.Partner_Tuning.getName());
...
//		productControl.scrollMouseUntilImageVisible(Constant.IMG_DRAG_DROP_AREA.Marketing_Drag_Drop_Area.getName());
		...
		// 7. Click "Retry" link
		// 8.Try to upload new tuning file again
		productControl.uploadFile(AddEditProductModel.RETRY_UPLOAD_MARKETING, AddEditProductModel.FileUpload.Tuning_HPXTT_LineOut.getName());
		/*
		 * Verify that New tuning file is uploaded successfully
		 */
	...
	/*
	 * Verify that the Headphone: X Tuning Rating  is hidden on the Variant Detail and Edit pages when the value equals "Undetermined."
	 */
	@Test
	public void TC052PMVE_09() {
		productControl.addLog("ID : TC052PMVE_09: Verify that the Headphone: X Tuning Rating  is hidden on the Variant Detail and Edit pages when the value equals 'Undetermined.' ");
		...
		7. Upload valid tuning successfully	
		8. Click "Request  Tuning Review" link	
		9. Log out DTS portal	
		10. Log into DTS portal as a DTS admin	
		11. Select above variant	
...
//		Create a aproved tuning product
...
//		11. Select above variant
		 ...
		 * VP: The Headphone: X Tuning Rating is Undetermined
		 */
		Assert.assertEquals(productControl.getTextByXpath(VariantInfo.TUNING_RATING),"Undetermined");
//		10. Click "Edit Version" link
...
//		14. Select above variant
		 ...
		 * VP: The Headphone: X Tuning Rating is A
		 */
		Assert.assertEquals(productControl.getTextByXpath(VariantInfo.TUNING_RATING),"A");
//		15. Click "Edit Version" link
		productControl.click(VariantInfo.EDIT_VARIANT);
		/*
		 * VP: The Headphone: X Tuning Rating is A
