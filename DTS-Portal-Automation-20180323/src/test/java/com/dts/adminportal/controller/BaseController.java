...
public class BaseController {
	...
	public void waitForAjax() {
        ...
        WebDriverWait wait = new WebDriverWait(driver, Constant.TIME_WAIT);
...
//			mouse.click(image.getCenter());
	...
	public boolean selectOptionByValue(String xpath, String value) {
		boolean result = false;
		if (value != null && !value.isEmpty()) {
			WebElement dropDownListBox = driver.findElement(By.xpath(xpath));
			Select clickThis = new Select(dropDownListBox);
			addLog("Select option: " + value);
	...
	public boolean isElementPresent(String xpath, String text) {
		try {
			...
			if (e.isDisplayed() && text.equals(e.getText())) {
				addLog("Element " + xpath + " displayed : " + text);
				return true;
			} else {
				addLog("Element " + xpath + " doesn't existed : " + text);
				return false;
			}
		} catch (NoSuchElementException e) {
			addLog("Element doesn't existed : " + xpath);
	...
	public boolean checkFullLicensedProductCombobox(String xpath) {
		ComboBoxElement comboBox= new ComboBoxElement(driver, By.xpath(xpath));
		comboBox.click();
		ArrayList<String> labels = comboBox.getLabelOptions();
		for (LicenseeProductType licenseeProductType : LicenseeProductType.values()) {
			if (!labels.contains(licenseeProductType.getName())) {
				addLog("Licensed Product Combobox is missing " + licenseeProductType.getName());
				return false;
			}
		}
		return true;
